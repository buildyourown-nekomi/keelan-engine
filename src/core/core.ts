...
import { execAsync } from "../utils/performance.js";
 ...
 */
export async function mountOverlayDirectory(lowerdir: string, upperdir: string, workdir: string, merge_path: string): Promise<void> {
    console.log(chalk.blue(`🔧 Mounting overlay: ${merge_path}`));
    try {
        await execAsync(`sudo mount -t overlay overlay -o lowerdir=${lowerdir},upperdir=${upperdir},workdir=${workdir} ${merge_path}`);
        console.log(chalk.green(`✅ Overlay mounted: ${merge_path}`));
    } catch (error) {
        console.error(chalk.red(`❌ Failed to mount overlay: ${error}`));
        throw error;
    }
}
 ...
 */
export async function mountBindDrive(path: string, mountpoint: string): Promise<void> {
    console.log(chalk.blue(`🔗 Binding drive: ${path} to ${mountpoint}`));
    try {
        await execAsync(`sudo mount --bind ${path} ${mountpoint}`);
        console.log(chalk.green(`✅ Bind drive mounted: ${mountpoint}`));
    } catch (error) {
        console.error(chalk.red(`❌ Failed to bind drive: ${error}`));
        throw error;
    }
}
 ...
 */
export async function checkMountpoint(mountpoint: string): Promise<boolean> {
    console.log(chalk.blue(`🔍 Checking mountpoint: ${mountpoint}`));
    try {
 ...
 */
export async function mountCrate(lowerdir: string, upperdir: string, workdir: string, merge_path: string): Promise<void> {

    if(await checkMountpoint(merge_path)) {
        console.log(chalk.yellow('⚠️  Mountpoint already exists'));
        return;
    }

    // Mount the directory
    console.log(chalk.yellow('🔧 Mounting OverlayFS...'));
    await mountOverlayDirectory(lowerdir, upperdir, workdir, merge_path);
    ...
    await mountBindDrive("/proc", merge_path + "/proc");
    await mountBindDrive("/dev", merge_path + "/dev");
    await mountBindDrive("/sys", merge_path + "/sys");

    // Mount DevPTS
    try {
        await execAsync(`mount -t devpts devpts ${merge_path}/dev/pts`);
    } catch (error) {
        console.error(chalk.red(`❌ Failed to mount devpts: ${error}`));
        throw error;
    }

    console.log(chalk.green('✅ System directories mounted'));
}
 ...
 */
export async function unmountCrate(merge_path: string): Promise<void> {
    
    if (!(await checkMountpoint(merge_path))) {
        console.log(chalk.yellow('⚠️  Mountpoint does not exist'));
        return;
    }
    ...
    try {
        await execAsync(`sudo umount ${merge_path}/dev/pts`);
        await execAsync(`sudo umount ${merge_path}/proc`);
        await execAsync(`sudo umount ${merge_path}/dev`);
        await execAsync(`sudo umount ${merge_path}/sys`);
    } catch (err: any) {
    ...
    try {
        await execAsync(`sudo umount ${merge_path}`);
        console.log(chalk.green('✅ OverlayFS unmounted'));
    } catch (err: any) {
        console.log(chalk.yellow(`❌ Failed to unmount OverlayFS: ${err.message}`));
    }

    console.log(chalk.green('✅ System directories unmounted'));
}
...
export async function removeCrate(name: string, options: { force?: boolean } = {}): Promise<void> {
    try {
        ...
        await unmountCrate(mergecratePath);
        
        console.log(chalk.yellow(`🧹 Removing crate: ${cratePath}`));
        await execAsync(`sudo rm -rf ${cratePath}`);
        await execAsync(`sudo rm -rf ${workcratePath}`);
        await execAsync(`sudo rm -rf ${mergecratePath}`);
        console.log(chalk.green(`✅ Crate removed: ${cratePath}`));
...
export async function removeGzipCrate(name: string, options: { force?: boolean } = {}): Promise<void> {
    try {
        ...
        await execAsync(`sudo rm -rf ${gzipCratePath}`);
        console.log(chalk.green(`✅ Gzip crate removed: ${gzipCratePath}`));
...
export async function removeShip(shipID: string, options: { force?: boolean; recursive?: boolean } = {}): Promise<void> {
    try {
        ...
        await execAsync(`sudo rm -rf ${workcratePath}`);
        await execAsync(`sudo rm -rf ${mergecratePath}`);
        console.log(chalk.green(`✅ Ship removed: ${cratePath}`));
    } catch (err: any) {
        if (!options.force) {
